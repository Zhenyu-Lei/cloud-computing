...
*/
int KV::kvSET(std::string & k, std::string & v)//设置/更新键值
{
    _kvdb[k] = v;
    return KV_OK;
}
...
*/
int KV::kvDEL(std::string & k)//删除单个 
{
...
int KV::kvDELArry(std::vector<std::string> & ks)//删除很多 
{
    int number = 0;
    for(auto & k : ks)
    {
...
*/
std::string KV::kvGET(std::string & k)//get 
{
    auto iter = _kvdb.find(k);
