...
#include "util.hpp"
//建立连接等等 
int Network::recvCommand(std::vector<std::string> & str)//接收命令（RESP形式） 
{
    int rc = KV_OK;
...
/*
    ...
    Therefor the coordinator should be responsible for the error
...
*/
int Network::reconnect()//如果有timeout或者network_close，coordinator 会认为参与者崩溃。这个过程：关闭过去的socket，然后recover 
{
    int rc = KV_OK;
